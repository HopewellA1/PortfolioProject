...
def viewtodo(request,todo_pk):
    ...
    if request.method == 'GET':
        forms = TodoForm(instance = todos)
        return render(request,'todo/viewtodo.html', {'todos':todos, 'forms':forms})
    else:
        try:
            forms = TodoForm(request.POST, instance = todos)
            forms.save()
            return redirect('current')
        except ValueError:
            return render(request,'todo/viewtodo.html', {'todos':todos, 'forms':forms})
...
def createtodo(request):
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request,'todo/createtodo.html',{'forms':TodoForm(), 'error':"Bad Title"})
